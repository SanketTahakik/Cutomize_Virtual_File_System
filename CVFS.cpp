...
void man(char *name)
{
    ...
    else if(strcmp(name,"write") == 0)
    {
        ...
        printf("Usege : read File_Name NO_of_Bytes_To_Read \n");
    }
    ...
    {
        printf("Description : Usage t display information of file\n");
        printf("Usage : stat File_Descriptor\n");
    }
    else if(strcmp(name,"truncate") == 0)
    {
        printf("Description : Usage to remove data from file\n");
...
int CreateFile(char * name, int permission)
{
    ...
    if(SUPERBLOCKobj.FreeInode)     return -2;
...
int ReadFile(int fd, char *arr, int isize)
{
    ...
    {
        strncpy(arr,(UFDTArr[fd].ptrfiletable -> ptrinode -> Buffer) + (UFDTArr->ptrfiletable -> readoffset), isize);
...
int main()
{
    ...
    while(1)
    {
        ...
        if(count == 1)
        {
            ...
            {
                printf("Terminating the Sanket Virtual File System\n");
                break;
            }
            else
            {
        ...
        else if(count == 2)
        {
            ...
            else if(strcmp(command[0], "truncate") == 0)
            {
                ...
                if(ret == -1)
                    printf("Errot : Incorrect parameter \n");
            }
            else
            {
                printf("\nError : command not fount !!!\n");
        ...
        else if(count == 3)
        {
            if(strcmp(command[0],"create") == 0)
            {
                ...
                if(ret >= 0)
                    printf("File is successfully create with file descriptior : %d\n", ret);
                if(ret == -1)
                    printf("Error : Incorrect parameter \n");
                if(ret == -2)
